...
public interface PerformanceMetricRepository extends JpaRepository<PerformanceMetric, Long> {
    ...
    @Query("SELECT p FROM PerformanceMetric p WHERE p.savedAt BETWEEN :startDate AND :endDate")
    List<PerformanceMetric> findBySavedAtBetween(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);
